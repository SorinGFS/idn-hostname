...
The validator uses a narrow ASCII fast path and otherwise deliberately separates UTS #46 preprocessing and IDNA2008 label validation from DNS presentation and length validation. Before entering the Unicode pipeline, `isIdnHostname` directly accepts a non-reserved ASCII LDH hostname with an optional terminal root dot when:
...
Each applicable `IdnaTestV2.txt` vector exercises both `isIdnHostname` and `idnHostname`; valid conversions must equal the expected nontransitional ToASCII result. Applicability excludes otherwise-valid `NV8`/`XV8` inputs permitted by default UTS #46 but rejected by this package's IDNA2008 policy. UTS #46 DNS-length verification is kept separate from the package's hostname validation: the `A4_1` and `A4_2` statuses associated with the optional `VerifyDnsLength` processing do not determine the package's DNS validity result. The package independently applies the DNS rules from RFC 1034 and RFC 1035: exactly one terminal empty label is accepted as the root, other empty labels are rejected, each non-root label is limited to 63 octets, and the complete wire-format domain name is limited to 255 octets. `U1` statuses are omitted because preprocessing uses `UseSTD3ASCIIRules=false`. CONTEXTO classification remains covered by the version-specific package fixtures rather than by the Unicode concern's applicability logic.
